            ...
            try:
                reply = await response.reply(
                    content=translation_manager.translate_random(
                        "response.api.cafe.ping.success", lang=locale
                    )
                )

                await reply.edit(
                    content=translation_manager.translate_random(
                        "response.api.cafe.ping.success_ms",
                        lang=locale,
                        ms=round((time.time() - ping_time) * 1000, 4)
                    )
                )
            except Exception:
                await self.reply_error(response, "response.cafe.ping.fail_unknown", locale)
